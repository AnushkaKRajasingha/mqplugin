 ...
 * @date 02/26/2015
 * @uses Pets Information
 *
 */
if (class_exists('MMQuoteBase') && !class_exists('MMPetPricing')) {
	class MMPetPricing extends MMQuoteBase {
		use MMDbBase;
		/**
		 * Cost for pets
		 * @name Cost for pets
		...
		public function __init(){
			try {
				$this->_setTablename('_petPricing');
			} catch (Exception $e) {
		...
		public function GetPetPricing(){
		try {
				
				$obj_array = (array) $this->Get_PetPricing();
		...
		public function Get_PetPricing($_isActive = 0){
			try {
		...
		public function CopyPetPricing(){
			try {
				$this->_validateId();
				$obj = $this->_getItem();
		...
		public function DeletePetPricing(){
			try {
				$this->_validateId();
				$obj = $this->_getItem();
				Plugin_Utilities::injectObjectData($obj, $this);
