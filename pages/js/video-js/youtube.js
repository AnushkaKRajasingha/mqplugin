...
(function() {
  ...
  videojs.Youtube = videojs.MediaTechController.extend({
    ...
    init: function(player, options, ready) {
      ...
      this.isIos = false;// /(iPad|iPhone|iPod)/g.test( navigator.userAgent );
      this.isAndroid = false;// /(Android)/g.test( navigator.userAgent );
      //used to prevent play events on IOS7 and Android > 4.2 until the user has clicked the player
	    //fixme have to be improved
      this.playVideoIsAllowed = true;//!(this.isIos || this.isAndroid);
  ...
  videojs.Youtube.prototype.addIframeBlocker = function(ieVersion) {
	...
	addEventListener(this.iframeblocker, 'touchstart', function(/*e*/) {
		if(self.paused()) {
            self.play();
        }
		else {
